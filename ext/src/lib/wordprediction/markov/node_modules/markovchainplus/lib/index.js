...
var MarkovChain = (function () {
  ...
  _createClass(MarkovChain, [{
    ...
    value: function process(predictTopThree = null) {
      ...
      //can pretty much change this to old version
      if (predictTopThree == null) {
        this.sentence = curWord;
        
        while ((this.wordBank[strings[0]] || this.wordBank2[strings[1]] || this.wordBank3[strings[2]]) && !this.endFn()) {
          if (this.wordBank3[curWord]) {
            curWord = pickOneByWeight(this.wordBank3[curWord]);
          }
          else if (this.wordBank2[twoWords]) {
            curWord = pickOneByWeight(this.wordBank2[twoWords]);
          }
          else {
            curWord = pickOneByWeight(this.wordBank[split[split.length-1]]);
          }
          this.sentence += ' ' + curWord;
        }
    ...
    value: function parse() {
      ...
      text.split(parseBy).forEach(function (lines) {
        ...
        for (var i = 0; i < words.length - 1; i++) {
           //clean this up - put all the if statements into its own funtion to not have redundant code
           var curWord = _this._normalize(words[i]);
           var nextWord = _this._normalize(words[i + 1]);
           var thirdWord = _this._normalize(words[i + 2]);
           var fourthWord = _this._normalize(words[i + 3]);
           var twoWords = curWord + " " + nextWord;
           var threeWords = curWord + " " + nextWord + " " + thirdWord;
