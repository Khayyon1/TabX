/* 
  Modified Markov chain prediction model: 
  Generate a wordbank for singular words as well as two and three consecutive words,
  & always return the word values with the highest probabilities for a particular key when predicting.
*/
...
var MarkovChain = (function () {
  ...
  _createClass(MarkovChain, [{
    ...
    value: function process(predict) {
      var input = this.startFn(this.wordBank);
      ...
      var i = 1;
      while (i < (splitStr.length + 1) && i < 4) {
        var curWord = splitStr[splitStr.length - i];
        ...
        if (i === 1) {
          ...
          curStr = curWord + " " + curStr; 
        }

        inputStrs.push(curStr);
        i++;
      }
      wordBanks.splice(0, (wordBanks.length - inputStrs.length));
      return predict(inputStrs.reverse(), wordBanks);
    }
  }, {
    key: 'parse',
    value: function parse() {
      ...
      text.split(parseBy).forEach(function (lines) {
        ...
        for (var i = 0; i < words.length - 3; i++) { 
          var startIndex = 0;
          if (i === 0) {
            words[i] = _this._normalize(words[i]);
            words[i + 1] = _this._normalize(words[i + 1]);
            words[i + 2] = _this._normalize(words[i + 2]);
          }
          else {
            startIndex = 3;
          }
          words[i + 3] = _this._normalize(words[i + 3]);

          // Parse current word list into substrings of 3, 2, and 1 words
          for (var j = startIndex; j < 4; j++) {
            if (words[i + j] !== undefined) {
              words[i + j] = words[i + j].replace(/’/g, "'");
              words[i + j] = words[i + j][0] + words[i + j].slice(1).toLowerCase();

              var upper = words[i + j][0].toUpperCase() + words[i + j].slice(1);
              if (alwaysCap.includes(upper)) {
                words[i + j] = upper;
              }
              else if (!alwaysCap.includes(upper) && /[A-Z]/.test(words[i + j][0])) {
                words[i + j] = undefined;
              }
            }
          }

          var twoWordStr = words[i] + " " + words[i + 1];
          var threeWordStr = words[i] + " " + words[i + 1] + " " + words[i + 2];
          _this.wordBank = _this.updateWordBank(_this.wordBank, words[i], words[i + 1]);
          _this.wordBank2 = _this.updateWordBank(_this.wordBank2, twoWordStr, words[i + 2]);
          _this.wordBank3 = _this.updateWordBank(_this.wordBank3, threeWordStr, words[i + 3]);
        }
      });
      return this;
    }
